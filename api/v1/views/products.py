...
from api.v1.serializers.products import (ProductBulkUpdateSerializer,
...
class ProductViewSet(viewsets.ModelViewSet):
    """ViewSet for the Product model with advanced filtering, searching, and custom actions."""
    ...
    def get_queryset(self):
        ...
        queryset = (
            Product.objects.all().select_related("shop").prefetch_related("categories")
        )
    ...
    def perform_create(self, serializer):
        ...
        if not user.is_staff and not user.shops.filter(id=shop_id).exists():
            self.permission_denied(
                self.request, message="You can only create products for shops you own."
            )
    ...
    def update_stock(self, request, pk=None):
        ...
        try:
            ...
            updated_product = ProductService.update_product_stock(
    ...
    def increment_view(self, request, pk=None):
        ...
        return Response(
            {"product_id": updated_product.id, "view_count": updated_product.view_count}
        )
    ...
    def bulk_update_prices(self, request):
        ...
        for product in products:
            if operation == "increase":
                factor = Decimal("1") + (
                    Decimal(price_change_percentage) / Decimal("100")
                )
            else:  # decrease
                factor = Decimal("1") - (
                    Decimal(price_change_percentage) / Decimal("100")
    ...
    def bulk_update_availability(self, request):
        ...
        return Response({"updated_count": updated_count, "is_available": is_available})
