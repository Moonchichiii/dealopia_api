...
class SearchView(APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="latitude", description="User latitude", type=OpenApiTypes.FLOAT
            ),
            OpenApiParameter(
                name="longitude", description="User longitude", type=OpenApiTypes.FLOAT
            ),
            OpenApiParameter(
                name="radius",
            ...
            OpenApiParameter(
                name="category", description="Category ID", type=OpenApiTypes.INT
            ),
            OpenApiParameter(
    ...
    def get(self, request):
        ...
        try:
            ...
            if query:
                ...
                data["local_results"]["categories"] = self._serialize_categories(
                    categories
                )
    ...
    def _extract_params(self, request):
        ...
        try:
            ...
            include_external = (
                request.GET.get("include_external", "true").lower() == "true"
            )
        except ValueError:
            return Response(
    ...
    def _perform_search(
        self, query, lat, lng, radius, category_id, min_sustainability, deals_filter
    ):
        ...
        if lat is not None and lng is not None:
            ...
            location_shops = ShopService.get_shops_by_location(
        ...
        return [
            {
                ...
                "distance": (
                    float(deal.distance.km) if getattr(deal, "distance", None) else None
                ),
        ...
        return [
            {
                ...
                "distance": (
                    float(shop.distance.km) if getattr(shop, "distance", None) else None
                ),
