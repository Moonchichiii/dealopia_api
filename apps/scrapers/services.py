            ...
            with sync_playwright() as p:
                ...
                with transaction.atomic():
                    for deal_data in deals:
                        if (
                            not deal_data["title"]
                            or deal_data["discountedPrice"]
                            >= deal_data["originalPrice"]
                        ):
                            continue
                        ...
                        if image_url and not image_url.startswith(
                            ...
                            image_url = (
                                f"{shop.website.rstrip('/')}/{image_url.lstrip('/')}"
                            )
                        ...
                        if product_url and not product_url.startswith(
                            ...
                            product_url = (
                                f"{shop.website.rstrip('/')}/{product_url.lstrip('/')}"
                            )

                        existing_deal = Deal.objects.filter(
                            shop=shop, title=deal_data["title"]
                        ).first()

                        if existing_deal:
                            existing_deal.original_price = deal_data["originalPrice"]
                            existing_deal.discounted_price = deal_data[
                                "discountedPrice"
                            ]
                            existing_deal.discount_percentage = deal_data[
                                "discountPercentage"
                            ]
