...
class Product(models.Model):
    ...
    def discounted_price(self):
        ...
        if self.discount_percentage > 0:
            discount_factor = Decimal('1') - (self.discount_percentage / 
                                             Decimal('100'))
    ...
    def get_active_deals(self):
        """Get all active deals for this product"""
        now = timezone.now()
        
        # Get product-specific deals
        product_deals = getattr(self, 'deals', models.QuerySet()).filter(
            is_verified=True,
            start_date__lte=now,
            end_date__gte=now
        )
        
        # Get category deals (if any) - check if categories is populated
        category_ids = self.categories.values_list('id', flat=True)
        if category_ids:
            category_deals = self.shop.deals.filter(
                is_verified=True,
                start_date__lte=now,
                end_date__gte=now,
            ...
            return product_deals | category_deals
