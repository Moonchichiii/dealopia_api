...
class ShopService:
    ...
    def get_popular_shops(limit=6):
        ...
        return (
            ...
            .filter(active_deal_count__gt=0)
    ...
    def get_shops_by_location(latitude, longitude, radius_km=10):
        ...
        return (
            ...
            .annotate(distance=F("location__point__distance_to")(user_location))
            .order_by("distance")
        )
    ...
    def get_shop_with_deals(shop_id):
        ...
        return {"shop": shop, "deals": active_deals}
    ...
    def calculate_shop_rating(shop_id):
        ...
        avg_rating = (
            Review.objects.filter(shop_id=shop_id, is_approved=True).aggregate(
    ...
    def get_shops_with_deals_in_category(category_id, limit=6):
        ...
        return (
            ShopService.get_verified_shops()
            .filter(deals__categories__id=category_id, deals__is_verified=True)
            .annotate(
                category_deal_count=Count(
                    "deals",
                    filter=Q(
                        deals__categories__id=category_id, deals__is_verified=True
                    ),
                )
            )
            .filter(category_deal_count__gt=0)
