...
class ShopAdmin(ShopOwnerMixin, ApiModelAdmin):
    ...
    menu_label = 'Shops'
    menu_icon = 'home'
    list_display = ('name', 'owner', 'is_verified', 'sustainability_score', 'created_at', 'website')
    list_filter = ('is_verified', 'sustainability_score')
    search_fields = ('name', 'description', 'short_description')
    inspect_view_enabled = True
    inspect_view_fields = [
        'id', 'name', 'description', 'website', 'owner', 'location',
        'rating', 'sustainability_score', 'created_at', 'updated_at'
    ]
...
class ProductAdmin(ShopOwnerMixin, ApiModelAdmin):
    ...
    menu_label = 'Products'
    menu_icon = 'tag'
    list_display = ('name', 'shop', 'price_display', 'stock_display', 'is_available', 'sustainability_score')
    list_filter = ('is_available', 'is_featured', 'shop')
    search_fields = ('name', 'description', 'sku', 'barcode')
    inspect_view_enabled = True
    inspect_view_fields = [
        'id', 'name', 'description', 'price', 'shop', 
    ...
    def stock_display(self, obj):
        ...
        if obj.stock_quantity <= 0:
            ...
            return format_html(
                '<span style="color: orange; font-weight: bold">Low: {}</span>', obj.stock_quantity
            )
        else:
...
class DealAdmin(ShopOwnerMixin, ApiModelAdmin):
    ...
    inspect_view_enabled = True
    inspect_view_fields = [
        'id', 'title', 'description', 'shop', 'original_price',
        'discounted_price', 'discount_percentage', 'start_date',
        'end_date', 'sustainability_score'
    ]
...
class ScraperJobAdmin(ApiModelAdmin):
    ...
    menu_icon = 'code'
    list_display = ('job_type', 'status', 'created_at', 'completed_at', 'sustainability_score')
